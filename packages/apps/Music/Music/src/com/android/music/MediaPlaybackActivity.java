...
//[HQ00056015]{proj:A1900L,type:Modify} add for keep awake when playing music start
import android.os.PowerManager;
import android.os.PowerManager.WakeLock;
//[HQ00056015]{proj:A1900L,type:Modify} add for keep awake when playing music end
    ...
    View.OnTouchListener, View.OnLongClickListener, CreateBeamUrisCallback
{
    ...
    public void onCreate(Bundle icicle) {
        ...
        /**M: ALPS01854283/Force to collapse search view and hide the IME panel. @**/
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (currentAudioId >= 0) {
            ...
            int search_src_text = searchView.getContext().getResources().getIdentifier("android:id/search_src_text", null, null);
            EditText searchsrctext = (EditText) searchView.findViewById(search_src_text);
            try {
                Class<?> mTextViewClass = searchsrctext.getClass().getSuperclass().getSuperclass().getSuperclass();
                Field mCursorDrawableRes = mTextViewClass.getDeclaredField("mCursorDrawableRes");
                mCursorDrawableRes.setAccessible(true);
                mCursorDrawableRes.set(searchsrctext, R.drawable.search_cursor);
            } catch(Exception e) {
            }

            int search_plate = searchView.getContext().getResources().getIdentifier("android:id/search_plate", null, null);
            View searchplate = searchView.findViewById(search_plate);
            searchplate.setBackgroundColor(android.graphics.Color.TRANSPARENT);

            int search_close = searchView.getContext().getResources().getIdentifier("android:id/search_close_btn", null, null);
            ImageView searchClose = (ImageView) searchView.findViewById(search_close);
            searchClose.setImageResource(R.drawable.delete_icon);
    ...
    private void toggleShuffle() {
        ...
        try {
            ...
            if (shuffle == MediaPlaybackService.SHUFFLE_NONE) {
				...
				//add by gaoyuhao @ 20150522 for HQ01042147 {
                ...
                //invalidateOptionsMenu();
              //  showToast(R.string.shuffle_off_notif);//modify by gaoyuhao @ 20150522 for HQ01042147
            } else {
    ...
    private void setPauseButtonImage() {
        try {
            if (mService != null && mService.isPlaying()) {
                mPauseButton.setImageResource(R.drawable.ic_shotcart_pause_btn_down);
                ...
                mPauseButton.setImageResource(R.drawable.ic_shotcart_play_btn_down);
            }
        } catch (RemoteException ex) {
        }
    }
