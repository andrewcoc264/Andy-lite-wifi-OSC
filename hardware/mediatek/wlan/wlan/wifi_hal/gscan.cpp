...
protected:
    virtual int handleResponse(WifiEvent& reply) {
		...
		struct nlattr *pdata = (struct nlattr *)data;
		void *payload = NULL;
		if(pdata->nla_type == NL80211_ATTR_VENDOR_CAPABILITIES)
		{
			payload = nla_data(pdata);
			len -= NLA_HDRLEN;
		}
		...
		ALOGI("%s: data=%p max_scan_cache_size=%d, %d, %d, %d, %d, %d, %d, max_bssid_history_entries=%d", __func__, data,
