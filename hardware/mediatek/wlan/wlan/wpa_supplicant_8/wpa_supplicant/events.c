			  ...
			  int only_first_ssid)
{
	unsigned int i;
#ifdef CONFIG_MTK_SCC
	struct wpa_ssid *temp_ssid = NULL;
	...
	for (i = 0; i < wpa_s->last_scan_res_used; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group,
		...
		if (!*selected_ssid)
			continue;
#ifdef CONFIG_MTK_SCC
		if (shared_freq > 0 && bss->freq != shared_freq) {
			wpa_printf(MSG_DEBUG, "bss channel different with p2p, skip it");
			temp_bss = bss;
			temp_ssid = *selected_ssid;
			continue;
		}
#endif
...
#ifdef CONFIG_MTK_SCC
	if (temp_ssid) {
		wpa_printf(MSG_DEBUG, "didn't find BSS on freq %d, return BSS on freq %d", shared_freq, temp_bss->freq);
		*selected_ssid = temp_ssid;
		return temp_bss;
	}
#endif
	return NULL;
